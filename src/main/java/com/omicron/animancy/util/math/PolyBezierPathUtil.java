package com.omicron.animancy.util.math;
...
public class PolyBezierPathUtil {
     ...
     */
    public static Path2D computePathThroughKnots(List<EPointF> knots) {
        throwExceptionIfInputIsInvalid(knots);

        final Path2D polyBezierPath = new Path2D.Float();
    ...
    private static EPointF[] computeControlPoints(int n, List<EPointF> knots) {
    ...
    private static EPointF[] constructTargetVector(int n, List<EPointF> knots) {
    ...
    private static void appendCurveToPath(Path2D path, EPointF control1, EPointF control2, EPointF targetKnot) {
        path.curveTo(
                control1.getX(),
                control1.getY(),
                control2.getX(),
